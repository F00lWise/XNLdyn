...
class XNLsim:
    ...
    def proc_ch_decay(self, R_core, rho_j):
        ...
        return (
                           core_holes.T * valence_resonant_occupation_share.T * valence_total_occupation_change).T / self.par.tau_CH
    ...
    def rate_j(self):
        ...
        without_scattering = self.res_inter - np.sum(self.nonres_inter, axis=2) - direct_augers - \
        ...
        with np.errstate(invalid='ignore'):
            energy_incoming = self.el_scatt * E_free / R_free  # This much energy is coming in
    ...
    def rate_E_free(self):
        ...
        result[interesting] = np.sum(self.nonres_inter[interesting] * self.par.energy_differences[interesting],
                                     axis=(1, 2)) \
                              + np.sum(self.ch_decay * (self.par.E_j + self.par.E_f), axis=1)[interesting] \
