...
if __name__ == '__main__':
    ...
    fluences_to_calculate = np.logspace(np.log10(np.min(fluences_photons_nm2)),
    ...
    if use_multiprocessing:
        ...
        with Pool(processes=10) as pool:
            tasklist = []
            """for fluence in fluences_to_calculate:
                tasklist.append(
    ...
    plt.figure()
    plt.plot(fluences_to_calculate,T,'.-', label = 'For one z-stack')
    ...
    for ipe, pulse_en in enumerate(final_pulse_energies):
        local_fluences, dA = calculate_fluences(Nsteps_r, pulse_en,
                                                  pulse_profile_sigma)
        local_transmitted = np.interp(local_fluences, fluences_to_calculate, tr[:,0])
        local_incidence_check = np.interp(local_fluences, fluences_to_calculate, inc[:,0])

        final_transmissions[ipe] = np.sum(local_transmitted*dA)
        final_incidence_check[ipe] = np.sum(local_incidence_check*dA)

    plt.figure()
    plt.plot(final_pulse_energies, final_transmissions*850*1e6*PAR.echarge, 'C1.-', label = 'For the entire spot')
    plt.plot(final_pulse_energies, final_incidence_check*850*1e6*PAR.echarge, 'C2.-', label = 'Incidences reproduced for reference')
